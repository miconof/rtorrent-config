...
set -e
########################################################
# rTorrent WebUI Installer 2010 May 14 Version 5.0 EXPERIMENTAL #
########################################################
...
function installRtorrentSvn {
	...
	sudo checkinstall -y || fail "Failed to install xmlrpc-c."
	...
	sudo checkinstall -y || fail "Failed to install libtorrent."
	cd ../rtorrent
	sudo ./autogen.sh
	sudo ./configure --with-xmlrpc-c || fail "Failed to configure rtorrent."
	sudo make || fail "Failed to make rtorrent."
	sudo checkinstall -y || fail "Failed to install rtorrent."
}
